...
function evaluate5(cards) {
  ...
  const straight = uniq.length === 5 && (uniq[0] - uniq[4] === 4 || JSON.stringify(uniq) === '[14,5,4,3,2]');
...
function bestOfSeven(cards) {
  return combinations(cards, 5).map(evaluate5).sort(compareHands).at(-1);
}
...
function highestBet(state) {
  return Math.max(...state.seats.map((s) => s.roundBet || 0));
}
...
function dealState(players, baseWager = 25) {
  const deck = makeDeck();
  const seats = players.slice(0, 6).map((p) => ({
    id: p.id,
    name: p.name,
    isBot: !!p.isBot,
    chips: 500 + (p.id === 'p1' ? baseWager : 0),
...
export function createInitialState({ mode, players, wager = 25 }) {
  ...
  state.status = `${state.seats[state.currentSeat].name} to act preflop`;
  return state;
}
...
function advanceStage(state) {
  ...
  state.status = `${state.seats[state.currentSeat]?.name || 'Next player'} to act on the ${state.stage}`;
  return state;
}
...
function finishAction(state) {
  ...
  state.status = `${state.seats[state.currentSeat]?.name || 'Next player'} to act`;
  return state;
}
...
export function handleAction(state, action) {
  ...
  if (action.type === 'fold') {
    seat.folded = true;
    seat.acted = true;
    next.status = `${seat.name} folds`;
    return finishAction(next);
  }
  if (action.type === 'check' && toCall === 0) {
    seat.acted = true;
    next.status = `${seat.name} checks`;
    return finishAction(next);
  }
  if (action.type === 'call') {
    commitBet(seat, toCall, next);
    next.status = `${seat.name} calls ${toCall}`;
    return finishAction(next);
  }
  if (action.type === 'bet') {
    const amount = Math.max(next.bigBlind, action.amount || next.bigBlind);
    commitBet(seat, amount, next);
    next.status = `${seat.name} bets ${amount}`;
    next.seats.forEach((s, idx) => {
      if (idx !== next.currentSeat && s.inHand && !s.folded) s.acted = false;
    });
    return finishAction(next);
  }
  if (action.type === 'raise') {
    const raiseTo = Math.max(highestBet(next) + next.minRaise, action.amount || highestBet(next) + next.minRaise);
    commitBet(seat, raiseTo - seat.roundBet, next);
    next.status = `${seat.name} raises to ${raiseTo}`;
    next.seats.forEach((s, idx) => {
      if (idx !== next.currentSeat && s.inHand && !s.folded) s.acted = false;
    });
    return finishAction(next);
  ...
  if (action.type === 'allin') {
    commitBet(seat, seat.chips, next);
    next.status = `${seat.name} is all-in`;
    next.seats.forEach((s, idx) => {
      if (idx !== next.currentSeat && s.inHand && !s.folded) s.acted = false;
    });
    return finishAction(next);
...
export function render(state, ctx) {
  ...
  const community = state.community.length ? state.community.map((c) => cardHtml(c)).join('') : '<div class="playing-card tiny"><div class="card-center">?</div></div>'.repeat(5);
  ...
  const seatsHtml = state.seats.map((s, idx) => `
    <div class="seat ${idx === state.currentSeat && !state.result ? 'active arcade-spark' : ''} ${s.folded ? 'folded' : ''}">
      ...
      <div class="mini-text">Chips: ${s.chips} · Round Bet: ${s.roundBet} · Total In: ${s.totalBet}</div>
      <div class="mini-text">${s.folded ? 'Folded' : s.allIn ? 'All-in' : 'Live'}</div>
      <div class="card-row">${s.cards.map((c) => cardHtml(c, s.id !== 'p1' && !state.revealAll && !state.result)).join('')}</div>
    </div>`).join('');
  wrap.innerHTML = `<div style="width:100%;"><div class="seat-row"><div class="match-banner">Pot: ${state.pot} · Stage: ${state.stage} · Turn: ${currentSeat?.name || '—'}</div></div><div class="market-board" style="margin:12px 0;">${community}</div><div class="seat-row">${seatsHtml}</div></div>`;
  root.append(wrap);
  const actingSeat = currentSeat;
  const toCall = Math.max(0, highestBet(state) - (actingSeat?.roundBet || 0));
  const humanTurn = !!currentSeat && !currentSeat.isBot;

  ctx.setInfo(`<div class="match-banner">${state.result || state.status}</div><div class="mini-text">Cash-game Hold'em with blinds, raises, all-ins, and hand evaluation.</div><div class="mini-text">Active seat: ${currentSeat?.name || '—'} · Pot: ${state.pot}</div>`);

  if (state.result) {
    ctx.setActions(`
      <div class="btn-row wrap">
        <button id="hRestart" class="action-btn primary" data-tip="Start a brand new Hold'em hand.">New Hand</button>
        <button id="hEnd" class="action-btn danger" data-tip="Clear the Hold'em table and return to the room.">End Game</button>
      </div>
    ...
    ctx.setActions(`
      <div class="btn-row wrap">
        <button id="hRestart" class="action-btn" data-tip="Restart this Hold'em table from the beginning.">New Hand</button>
        <button id="hEnd" class="action-btn danger" data-tip="Clear the Hold'em table and return to the room.">End Game</button>
      </div>
      <div class="mini-text">Waiting for ${currentSeat?.name || 'the next player'} to act… bot turns should resolve automatically.</div>
    `);
  } else {
    ctx.setActions(`
      <div class="btn-row wrap">
        <button id="hFold" class="action-btn" data-tip="Fold this hand and surrender your claim to the pot.">Fold</button>
        <button id="hCheck" class="action-btn" data-tip="Check when no chips are required to stay in.">Check</button>
        <button id="hCall" class="action-btn primary" data-tip="Match the current highest bet and stay in the hand.">Call ${toCall}</button>
        <button id="hBet" class="action-btn" data-tip="Open the betting with a standard value bet.">Bet ${state.bigBlind * 2}</button>
        <button id="hRaise" class="action-btn" data-tip="Increase the current wager and force the table to respond.">Raise</button>
        <button id="hAllIn" class="action-btn warn" data-tip="Push all of your remaining chips into the pot.">All-in</button>
        <button id="hRestart" class="action-btn" data-tip="Restart this Hold'em table from the beginning.">New Hand</button>
        <button id="hEnd" class="action-btn danger" data-tip="Clear the Hold'em table and return to the room.">End Game</button>
      </div>
      <div class="mini-text">Only the active seat may act. Solo and share modes fill extra seats with Buddy Bots.</div>
    `);
  }

  queueMicrotask(() => {
    const fire = (action) => {
      const next = handleAction(state, action);
      ctx.onStateChange(next, true);
      if (!next.result) ctx.autoBotTurn?.();
    };
    document.getElementById('hFold')?.addEventListener('click', () => fire({ type: 'fold' }));
    document.getElementById('hCheck')?.addEventListener('click', () => fire({ type: 'check' }));
    document.getElementById('hCall')?.addEventListener('click', () => fire({ type: 'call' }));
    document.getElementById('hBet')?.addEventListener('click', () => fire({ type: 'bet', amount: state.bigBlind * 2 }));
    document.getElementById('hRaise')?.addEventListener('click', () => fire({ type: 'raise', amount: highestBet(state) + state.bigBlind * 2 }));
    document.getElementById('hAllIn')?.addEventListener('click', () => fire({ type: 'allin' }));
    document.getElementById('hRestart')?.addEventListener('click', () => ctx.restartMatch?.());
    document.getElementById('hEnd')?.addEventListener('click', () => ctx.endMatch?.(null, 'Cleared the Texas Hold’em table.'));
  });
  return root;
}
