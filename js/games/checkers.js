...
function crownIfNeeded(piece, row) {
  if ((piece.color === 'white' && row === 0) || (piece.color === 'black' && row === 7)) piece.king = true;
}
...
export function render(state, ctx) {
  ...
  for (let r = 0; r < 8; r++) for (let c = 0; c < 8; c++) {
    ...
    const piece = state.board[r][c];
    cell.innerHTML = piece ? `<span class="piece">${ICON[piece.color][piece.king]}</span>` : '';
  ...
  ctx.setActions(`
    <div class="btn-row wrap">
      <button id="checkNew" class="action-btn primary" data-tip="Start a fresh checkers game with the same mode and wager.">New Game</button>
      <button id="checkEnd" class="action-btn danger" data-tip="Clear the checkers board and return to the main room.">End Game</button>
    </div>
    <div class="mini-text">Select a checker, then a highlighted square. If you jump and another jump is available, you must keep going.</div>
  `);
  queueMicrotask(() => {
    document.getElementById('checkNew')?.addEventListener('click', () => ctx.restartMatch?.());
    document.getElementById('checkEnd')?.addEventListener('click', () => ctx.endMatch?.(null, 'Cleared the checkers board.'));
  });
  return root;
}
