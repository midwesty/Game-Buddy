...
function newHand(stateBase) {
  const deck = makeDeck();
  const seats = stateBase.players.map((p) => ({ ...p, hand: Array.from({ length: 6 }, () => deck.pop()).sort((a, b) => rankValue(a.rank) - rankValue(b.rank)), won: [] }));
  return { ...stateBase, deck, seats, phase: 'bidding', currentSeat: (stateBase.dealerIndex + 1) % 4, bidHistory: [], highestBid: null, bidderSeat: null, trump: null, trick: [], trickLeader: null, handNumber: (stateBase.handNumber || 0) + 1, status: `${seats[(stateBase.dealerIndex + 1) % 4].name} opens the bidding` };
}
...
function scoreHand(state) {
  ...
  if (trumpCardsWon.length) {
    ...
    teamPoints[sorted.at(-1).team] += 1;
    teamPoints[sorted[0].team] += 1;
    const jack = trumpCardsWon.find((x) => x.card.rank === 'J');
    if (jack) teamPoints[jack.team] += 1;
  }
...
export function handleAction(state, action) {
  ...
  if (next.phase === 'bidding') {
    if (action.type === 'bid') {
      ...
      if (!next.highestBid || action.amount > next.highestBid.amount) {
        next.highestBid = { seat: next.currentSeat, amount: action.amount };
        next.bidderSeat = next.currentSeat;
      }
    } else if (action.type === 'pass') next.bidHistory.push({ seat: next.currentSeat, amount: 0 });
    if (next.bidHistory.length >= 4) {
      if (!next.highestBid) {
        next.highestBid = { seat: (next.dealerIndex + 1) % 4, amount: 2 };
        next.bidderSeat = (next.dealerIndex + 1) % 4;
      }
      next.phase = 'chooseTrump';
      next.currentSeat = next.bidderSeat;
      next.status = `${next.seats[next.bidderSeat].name} chooses trump`;
      return next;
    }
    next.currentSeat = (next.currentSeat + 1) % 4;
    next.status = `${next.seats[next.currentSeat].name} to bid`;
    return next;
  }
  ...
  if (next.phase === 'chooseTrump' && action.type === 'trump') {
    ...
    next.phase = 'play';
    next.currentSeat = next.bidderSeat;
    next.trickLeader = next.bidderSeat;
    next.status = `${next.seats[next.currentSeat].name} leads`;
    return next;
  }
  ...
  if (next.phase === 'play' && action.type === 'playCard') {
    ...
    if (next.trick.length === 4) {
      ...
      next.status = `${next.seats[winner.seat].name} won the trick and leads next`;
    ...
    next.currentSeat = (next.currentSeat + 1) % 4;
    next.status = `${next.seats[next.currentSeat].name} to play`;
    return next;
  }
...
export function render(state, ctx) {
  ...
  const hands = state.seats.map((s, idx) => `
    <div class="seat ${idx === state.currentSeat && !state.result ? 'active arcade-spark' : ''}">
      ...
      <div class="card-row">${s.hand.map((c, i) => `<button class="card-btn" data-seat="${idx}" data-index="${i}" data-tip="Play ${c.rank}${c.suit}" style="border:0;background:transparent;padding:0;">${cardHtml(c, s.id !== 'p1' && !state.revealAll && !state.result)}</button>`).join('')}</div>
    </div>`).join('');
  wrap.innerHTML = `<div style="width:100%;"><div class="seat-row"><div class="match-banner">Team 1: ${state.teamScores[0]} · Team 2: ${state.teamScores[1]} · Trump: ${state.trump || '—'} · Turn: ${currentSeat?.name || '—'}</div></div><div class="seat-row" style="margin:12px 0;">${trickHtml}</div><div class="seat-row">${hands}</div></div>`;
  root.append(wrap);
  ...
  if (state.result) {
    ctx.setActions(`
      <div class="btn-row wrap">
        <button id="pitchNew" class="action-btn primary" data-tip="Start a fresh Pitch match.">New Game</button>
        <button id="pitchEnd" class="action-btn danger" data-tip="Clear the Pitch table and return to the room.">End Game</button>
      </div>
    ...
    ctx.setActions(`
      <div class="btn-row wrap">
        <button id="pitchNew" class="action-btn" data-tip="Restart this Pitch match from the opening hand.">New Game</button>
        <button id="pitchEnd" class="action-btn danger" data-tip="Clear the Pitch table and return to the room.">End Game</button>
      </div>
      <div class="mini-text">Waiting for ${currentSeat?.name || 'the next player'}… bot turns should resolve automatically.</div>
    `);
  } else if (state.phase === 'bidding') {
    const minBid = Math.max(2, (state.highestBid?.amount || 1) + 1);
    ctx.setActions(`
      <div class="btn-row wrap">
        <button id="pitchPass" class="action-btn" data-tip="Pass and leave the current high bid alone.">Pass</button>
        <button id="pitchBid2" class="action-btn primary" data-tip="Make the next legal bid.">Bid ${minBid}</button>
        <button id="pitchBid3" class="action-btn" data-tip="Jump the bidding one higher than the minimum.">Bid ${minBid + 1}</button>
        <button id="pitchNew" class="action-btn" data-tip="Restart this Pitch match from the opening hand.">New Game</button>
        <button id="pitchEnd" class="action-btn danger" data-tip="Clear the Pitch table and return to the room.">End Game</button>
      </div>
      <div class="mini-text">Current high bid: ${state.highestBid?.amount || 'none'}.</div>
    `);
  } else if (state.phase === 'chooseTrump') {
    ctx.setActions(`
      <div class="btn-row wrap">${SUITS.map((s) => `<button class="action-btn" data-trump="${s}" data-tip="Choose ${s} as trump.">${s}</button>`).join('')}<button id="pitchNew" class="action-btn" data-tip="Restart this Pitch match from the opening hand.">New Game</button><button id="pitchEnd" class="action-btn danger" data-tip="Clear the Pitch table and return to the room.">End Game</button></div>
      <div class="mini-text">Bid winner chooses trump.</div>
    `);
    ...
    ctx.setActions(`
      ...
      <div class="mini-text">Follow suit if you can. If you cannot, you may throw off or play trump.</div>
    `);
  } else {
  ...
  queueMicrotask(() => {
    ...
    document.getElementById('pitchPass')?.addEventListener('click', () => fire({ type: 'pass' }));
    document.getElementById('pitchBid2')?.addEventListener('click', () => fire({ type: 'bid', amount: Math.max(2, (state.highestBid?.amount || 1) + 1) }));
    document.getElementById('pitchBid3')?.addEventListener('click', () => fire({ type: 'bid', amount: Math.max(3, (state.highestBid?.amount || 1) + 2) }));
    document.querySelectorAll('[data-trump]').forEach((el) => el.addEventListener('click', () => fire({ type: 'trump', suit: el.dataset.trump })));
    document.querySelectorAll('.card-btn').forEach((el) => el.addEventListener('click', () => {
      const seatIndex = Number(el.dataset.seat);
      if (state.seats[seatIndex]?.isBot || seatIndex !== state.currentSeat) return;
      fire({ type: 'playCard', index: Number(el.dataset.index) });
    }));
    document.getElementById('pitchNew')?.addEventListener('click', () => ctx.restartMatch?.());
    document.getElementById('pitchEnd')?.addEventListener('click', () => ctx.endMatch?.(null, 'Cleared the Pitch table.'));
  });
